...
export const paypalWebhook: RequestHandler = async (req, res) => {
  try {
    ...
    if (event.event_type !== 'PAYMENT.CAPTURE.COMPLETED') return res.json({ ok: true });

    const resource = event.resource || {};
    const paymentId = resource.id;
    const amount = Number(resource.amount?.value || 0);
    const currency = resource.amount?.currency_code || 'EUR';
    const customId = resource.supplementary_data?.related_ids?.order_id || resource.custom_id || null;
    ...
    if (uid) {
      ...
      batch.set(userRef, { 'balances.available': FieldValue.increment(amount) }, { merge: true });
    }
