...
function TicketsPage() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "tickets"), (snap) => setTickets(snap.docs.map((d) => ({ id: d.id, ...d.data() }))));
    return () => unsub();
  }, []);
    ...
    <div className="container py-10">
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <div className="rounded-xl border border-border/60 bg-card p-4 space-y-2">
          <input className="w-full rounded-md bg-background px-3 py-2 border border-border/60" placeholder="Titre" value={title} onChange={(e) => setTitle(e.target.value)} />
          <textarea className="w-full rounded-md bg-background px-3 py-2 border border-border/60" placeholder="Message" value={msg} onChange={(e) => setMsg(e.target.value)} />
